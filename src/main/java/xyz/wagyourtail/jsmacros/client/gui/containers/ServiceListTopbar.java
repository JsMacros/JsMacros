...
public class ServiceListTopbar extends MultiElementContainer<ServiceScreen> {
    ...
    public void init() {
        ...
        addDrawableChild(new Button(x + w * 10 / 12 + 1, y + 1, w / 12 - 1, height - 3, textRenderer, 0, 0xFF000000, 0x7F7F7F7F, 0xFFFFFF, new TranslatableText("jsmacros.enabled"), (btn) -> {
            //TODO: sort by enabled
            parent.reload();
        }));

        addDrawableChild(new Button(x + w * 11 / 12 + 1, y + 1, w / 12 - 1, height - 3, textRenderer, 0, 0xFF000000, 0x7F7F7F7F, 0xFFFFFF, new TranslatableText("jsmacros.running"), (btn) -> {
        ...
        addDrawableChild(new Button(x + w - 1, y+1, 11, height - 3, textRenderer, 0, 0xFF000000, 0x7F7F7F7F, 0xFFFFFFFF, new LiteralText("+"), (btn) -> {
            openOverlay(new TextPrompt(parent.width / 4, parent.height / 4, parent.width / 2, parent.height / 2, textRenderer, new TranslatableText("jsmacros.servicename"), "", getFirstOverlayParent(), (name) -> {
                if (Core.instance.services.registerService(name, new ServiceTrigger(Core.instance.config.macroFolder, false)))
