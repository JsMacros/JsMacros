...
public class BossBarConsumer implements BossBarS2CPacket.Consumer {
    public void add(UUID uuid, Text name, float percent, BossBar.Color color, BossBar.Style style, boolean darkenSky, boolean dragonMusic, boolean thickenFog) {
        ...
        new EventBossbar("ADD", uuid, bar);
    }

    public void remove(UUID uuid) {
        new EventBossbar("REMOVE", uuid, null);
    }

    public void updateProgress(UUID uuid, float percent) {
        ClientBossBar bar = ((IBossBarHud) MinecraftClient.getInstance().inGameHud.getBossBarHud()).jsmacros_GetBossBars().get(uuid);
        new EventBossbar("UPDATE_PERCENT", uuid, bar);
    }

    public void updateName(UUID uuid, Text name) {
        ClientBossBar bar = ((IBossBarHud) MinecraftClient.getInstance().inGameHud.getBossBarHud()).jsmacros_GetBossBars().get(uuid);
        new EventBossbar("UPDATE_NAME", uuid, bar);
    }

    public void updateStyle(UUID id, BossBar.Color color, BossBar.Style style) {
        ClientBossBar bar = ((IBossBarHud) MinecraftClient.getInstance().inGameHud.getBossBarHud()).jsmacros_GetBossBars().get(id);
        new EventBossbar("UPDATE_STYLE", id, bar);
    }
