...
public class ServiceListTopbar extends MultiElementContainer<ServiceScreen> {
    ...
    public void init() {
        ...
        addButton(new Button(x + 1, y + 1, w * 2 / 12 - 1, height - 3, textRenderer, 0x3FFFFFFF, 0xFF000000, 0x7F7F7F7F, 0xFFFFFF, new TranslatableText("jsmacros.servicename"), (btn) -> {
            //TODO: sort by name
            parent.reload();
        }));

        addButton(new Button(x + w * 2 / 12 + 1, y + 1, w * 8 / 12 - 1, height - 3, textRenderer, 0x3FFFFFFF, 0xFF000000, 0x7F7F7F7F, 0xFFFFFF, new TranslatableText("jsmacros.file"), (btn) -> {
            //TODO: sort by file
            parent.reload();
        }));

        addButton(new Button(x + w * 10 / 12 + 1, y + 1, w / 12 - 1, height - 3, textRenderer, 0, 0xFF000000, 0x7F7F7F7F, 0xFFFFFF, new TranslatableText("jsmacros.enabledstatus"), (btn) -> {
            //TODO: sort by enabled
            parent.reload();
        }));

        addButton(new Button(x + w * 11 / 12 + 1, y + 1, w / 12 - 1, height - 3, textRenderer, 0, 0xFF000000, 0x7F7F7F7F, 0xFFFFFF, new TranslatableText("jsmacros.runningstatus"), (btn) -> {
