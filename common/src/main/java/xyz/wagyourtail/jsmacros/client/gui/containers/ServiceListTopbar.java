...
public class ServiceListTopbar extends MultiElementContainer<ServiceScreen> {
    ...
    public void init() {
        ...
        addDrawableChild(new Button(x + 1, y + 1, w * 2 / 12 - 1, height - 3, textRenderer, 0x3FFFFFFF, 0xFF000000, 0x7F7F7F7F, 0xFFFFFF, Text.translatable("jsmacros.servicename"), (btn) -> {
            //TODO: sort by name
            parent.reload();
        }));

        addDrawableChild(new Button(x + w * 2 / 12 + 1, y + 1, w * 8 / 12 - 1, height - 3, textRenderer, 0x3FFFFFFF, 0xFF000000, 0x7F7F7F7F, 0xFFFFFF, Text.translatable("jsmacros.file"), (btn) -> {
            //TODO: sort by file
            parent.reload();
        }));

        addDrawableChild(new Button(x + w * 10 / 12 + 1, y + 1, w / 12 - 1, height - 3, textRenderer, 0, 0xFF000000, 0x7F7F7F7F, 0xFFFFFF, Text.translatable("jsmacros.enabledstatus"), (btn) -> {
            //TODO: sort by enabled
            parent.reload();
        }));

        addDrawableChild(new Button(x + w * 11 / 12 + 1, y + 1, w / 12 - 1, height - 3, textRenderer, 0, 0xFF000000, 0x7F7F7F7F, 0xFFFFFF, Text.translatable("jsmacros.runningstatus"), (btn) -> {
